        ...
        <div className="py-2">
          {results.map((result, idx) => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-4 py-3 transition-colors group text-left ${focusedIndex === idx ? "bg-zinc-800" : "hover:bg-zinc-900"
                }`}
            >
        ...
        <div className="container mx-auto px-4 md:px-6 flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 md:gap-6">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.name}
                    href={link.href}
                    className={`text-xs font-bold uppercase tracking-widest transition-all ${isActive
                        ? "text-amber-500"
            ...
            <Link
              ...
              className={`p-2 transition-all rounded-full ${authenticated
                    ...
                    <div className="py-2">
                      {results.map((result, idx) => (
                        <button
                          key={result.href + result.title + idx}
                          onMouseDown={(e) => { e.preventDefault(); navigateTo(result.href); }}
                          className={`w-full flex items-center gap-3 px-4 py-3 transition-colors text-left ${focusedIndex === idx ? "bg-zinc-800" : "hover:bg-zinc-900"
                            }`}
                        >
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`flex flex-col items-center gap-1.5 transition-all group ${isActive ? "text-amber-500" : "text-zinc-500 hover:text-white"
                    }`}
                >
